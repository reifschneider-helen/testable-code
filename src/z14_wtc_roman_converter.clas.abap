CLASS z14_wtc_roman_converter DEFINITION
   PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    CONSTANTS:
         error_value TYPE i VALUE -1 ##NO_TEXT.
  ...
  PRIVATE SECTION.
    METHODS:
      value_of_roman_digit
        IMPORTING
          i_roman_numeral_digit TYPE ty_char1
        RETURNING
          VALUE(r_arabic)       TYPE i.
ENDCLASS.

CLASS z14_wtc_roman_converter IMPLEMENTATION.


  METHOD to_arabic.
    DATA:
      offset              TYPE i,
      current_digit       TYPE ty_char1,
      previous_digit      TYPE ty_char1,
      roman               TYPE string,
...
* - subtractive: only one valid cases: I before X|V, X before L|C, C before D|M
* Added specific error handling checks
    ...
    DO strlen( roman ) TIMES.
      ...
      IF value_of_prev_digit > 0 AND value_of_prev_digit < value_of_cur_digit.
        ...
        " CHECK: valid cases: I before X|V, X before L|C, C before D|M
        CASE current_digit.
      ...
      "-- Check single-use digits (V, L, D cannot repeat consecutively)
      IF current_digit = previous_digit AND
         ( current_digit = 'V' OR current_digit = 'L' OR current_digit = 'D' ).
        r_arabic = error_value.
        EXIT.
      ENDIF.
